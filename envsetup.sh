...
- auremote: Add git remote for AU Gerrit Review
- augerrit: A Git wrapper that fetches/pushes patch from/to AU Gerrit Review
- aurebase: Rebase a Gerrit change and push it again
- aospremote: Add git remote for matching AOSP repository
- cafremote: Add git remote for matching CodeAurora repository.
- mka:      Builds using SCHED_BATCH on all processors
- mkap:     Builds the module(s) using mka and pushes them to the device.
- cmka:     Cleans and builds using mka.
- pspush:   push commit to AU gerrit instance.
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^au_") ; then
       AU_DEVICE=$(echo -n $1 | sed -e 's/^au_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $AU_DEVICE; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       AU_DEVICE=
    fi
    export AU_DEVICE
...
function breakfast()
{
    target=$1
    local variant=$2
    AU_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/au/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the AU model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch au_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    local device=$(echo -n $product | sed -e "s/.*au_//")
    check_product $product
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the AU github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var AU_VERSION)
        ZIPFILE=au-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.au.device | grep -q "$AU_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $AU_BUILD, run away!"
    fi
}
...
function mbot() {
    unset LUNCH_MENU_CHOICES
    croot
    ./vendor/au/bot/deploy.sh
}

function pspush_host() {
    echo ""
    echo "Host au_gerrit"
    echo "  Hostname gerrit.au.co"
    echo "  Port 29418"
    echo "  User $1"
...
function pspush_host_create() {
    echo "Please enter sshusername registered with gerrit.au.co."
    read sshusername
...
function pspush() {
    local project
    project=`git config --get remote.au.projectname`
    ...
    if [ -z "$1" ] || [ "$1" = '--help' ]; then
        ...
        echo "will execute 'git push ssh://\$sshusername@gerrit.au.co:29418/$project HEAD:refs/[for][drafts][heads]/$revision'"
    else
        check_ssh_config="`grep -A 1 'gerrit$' ~/.ssh/config`"
        check_ssh_config_2=`echo "$check_ssh_config" | while read line; do grep gerrit.au.co; done`
        if [ -n "$check_ssh_config" ]; then
            if [ -n "$check_ssh_config_2" ]; then
                git push au_gerrit:$project HEAD:refs/$1/$revision
            fi
...
function taco() {
    ...
    do
        ...
        if [ $? -eq 0 ]; then
            croot
            ./vendor/au/bot/build_device.sh au_$sauce-userdebug $sauce
        else
            echo "No such item in brunch menu. Try 'breakfast'"
        fi
    done
...
function addau() {
    ...
    REPO=$(cat .git/config  | grep git://github.com/aurumOS/ | awk '{ print $NF }' | sed s#git://github.com/##g)
    if [ -z "$REPO" ]
    then
        REPO=$(cat .git/config  | grep https://github.com/aurumOS/ | awk '{ print $NF }' | sed s#https://github.com/##g)
        if [ -z "$REPO" ]
        then
    ...
    AUUSER=`git config --get review.gerrit.au.co.username`
    if [ -z "$AUUSER" ]
    then
        git remote add gerrit ssh://gerrit.au.co:29418/$REPO
    else
        git remote add gerrit ssh://$AUUSER@gerrit.au.co:29418/$REPO
    fi
    if ( git remote -v | grep -qv gerrit ) then
        echo "AU gerrit $REPO remote created"
    else
...
function auremote()
{
    ...
    git remote rm auremote 2> /dev/null
    GERRIT_REMOTE=$(git config --get remote.github.projectname)
    AUUSER=$(git config --get review.gerrit.au.co.username)
    if [ -z "$AUUSER" ]
    then
        git remote add auremote ssh://gerrit.au.co/:29418/$GERRIT_REMOTE
    else
        git remote add auremote ssh://$AUUSER@gerrit.au.co:29418/$GERRIT_REMOTE
    fi
    echo "Remote 'auremote' created"
}
...
function makerecipe() {
  ...
  then
    pwd
    auremote
    git push auremote HEAD:refs/heads/'$1'
  fi
  '
}
...
function augerrit() {
    ...
    local user=`git config --get review.gerrit.au.co.username`
...
function aurebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AU Gerrit Rebase Usage: "
        echo "      aurebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://gerrit.au.co/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function reposync() {
    case `uname -s` in
        Darwin)
            if [[ $AU_REPOSYNC_QUIET = true ]]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.au.device | grep -q "$AU_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/au/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
