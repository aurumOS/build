#!/usr/bin/env python
# Copyright (C) 2016, The aurumOS Project
...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from aurumOS Github (http://github.com/aurumOS)." % device)
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('aurumOS/%s already fetched to %s' % (repo_name, repo_target))
            continue

        print('Adding dependency: auOSP/%s -> %s' % (repo_name, repo_target))
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "auOSP/%s" % repo_name })
...
def fetch_dependencies(repo_path, fallback_branch = None):
    ...
    dependencies_path = repo_path + '/au.dependencies'
    syncable_repos = []
...
print("Repository for %s not found in the auOSP Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
